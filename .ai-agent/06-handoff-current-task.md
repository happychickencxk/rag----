...
## 2026-06-29 本地 Mock 联调与验收

- 新增并修正 `mock-server/server.js`，按接口文档提供统一响应、分页、Bearer Token 鉴权、刷新令牌、SSE、会话、引用、反馈和文档接口。
- 新增 `mock-server/contract.test.js`，使用随机端口执行完整契约验收，不依赖已经启动的 8000 端口服务。
- 小程序请求层修复 401 重放上限、上传刷新函数导出、历史删除方法引用和 API 地址规范化。
- 适配层兼容正式字段：`kb_id`、`session_id`、`message_id`、`chunk_id`、`doc_name`、`chapter_path`、`similarity_score` 等。
- SSE 改为有状态 UTF-8 解码，已用真实字节中间拆包测试，中文不再出现替换字符。
- 登录页恢复 `wx.login` 和 `/auth/wechat-login`，问答页等待知识库列表返回后再恢复默认知识库。
- 修复历史时间的 UTC 转本地时区、引用日期展示和反馈页上下文补取；反馈摘要改为 JS 预计算，避免 WXML 调用字符串方法得到 `undefined`。
- 文件中心保留待选文件真实大小，上传成功后仍不提供下载，因为接口文档没有安全下载 URL。
- 自动测试共 53 项通过；微信开发者工具已实际验收登录、知识库、SSE、历史、引用、反馈和文件中心。
- 下一步只应连接真实后端进行差异联调，不要把本地 Mock 通过表述为生产后端通过。
...
- SSE 解析器已在阶段1完成（`utils/sse.js`），处理拆包、粘包、CRLF/LF、中文 UTF-8 边界、心跳行和 done 事件。
- 流式输出期间显示"正在生成回答..."指示器和闪烁光标。
- 流式完成时保存 message_id 和 citations，自动解析段落结构。
- SSE 失败且有部分内容时保留已流式文本；无内容时回退到非流式问答。
- 页面卸载、切换知识库或新提问时取消旧 SSE 请求（`abort()`）。
